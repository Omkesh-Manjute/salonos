...
create policy if not exists "owners_view_their_salon" on public.salons
for select using (tenant_id = public.current_tenant_id());
create policy if not exists "owners_update_their_salon" on public.salons
...
create policy if not exists "admins_manage_users" on public.users
for all using (public.current_user_role() = 'admin') with check (public.current_user_role() = 'admin');
create policy if not exists "users_view_same_tenant_users" on public.users
...
create policy if not exists "admins_manage_services" on public.services
for all using (public.current_user_role() = 'admin') with check (public.current_user_role() = 'admin');
create policy if not exists "tenant_services_visible" on public.services
for select using (tenant_id = public.current_tenant_id());
create policy if not exists "owners_manage_services" on public.services
...
create policy if not exists "admins_manage_bookings" on public.bookings
for all using (public.current_user_role() = 'admin') with check (public.current_user_role() = 'admin');
create policy if not exists "tenant_bookings_visible" on public.bookings
for select using (tenant_id = public.current_tenant_id() or user_id = auth.uid());
create policy if not exists "customers_create_own_bookings" on public.bookings
for insert with check (user_id = auth.uid() and tenant_id = public.current_tenant_id());
create policy if not exists "owners_manage_tenant_bookings" on public.bookings
for all using (tenant_id = public.current_tenant_id() and public.current_user_role() = 'owner') with check (tenant_id = public.current_tenant_id() and public.current_user_role() = 'owner');

create policy if not exists "admins_manage_queue" on public.queue
for all using (public.current_user_role() = 'admin') with check (public.current_user_role() = 'admin');
create policy if not exists "tenant_queue_visible" on public.queue
for select using (tenant_id = public.current_tenant_id() or user_id = auth.uid());
create policy if not exists "customers_join_queue" on public.queue
for insert with check (tenant_id = public.current_tenant_id() and (user_id = auth.uid() or public.current_user_role() = 'owner'));
create policy if not exists "owners_manage_tenant_queue" on public.queue
for all using (tenant_id = public.current_tenant_id() and public.current_user_role() = 'owner') with check (tenant_id = public.current_tenant_id() and public.current_user_role() = 'owner');

create policy if not exists "admins_manage_subscriptions" on public.subscriptions
for all using (public.current_user_role() = 'admin') with check (public.current_user_role() = 'admin');
create policy if not exists "tenant_subscription_visible" on public.subscriptions
for select using (tenant_id = public.current_tenant_id());
create policy if not exists "owners_manage_subscription" on public.subscriptions
for all using (tenant_id = public.current_tenant_id() and public.current_user_role() = 'owner') with check (tenant_id = public.current_tenant_id() and public.current_user_role() = 'owner');

create policy if not exists "admins_manage_notifications" on public.notifications
for all using (public.current_user_role() = 'admin') with check (public.current_user_role() = 'admin');
create policy if not exists "tenant_notifications_visible" on public.notifications
for select using (tenant_id = public.current_tenant_id() or user_id = auth.uid());

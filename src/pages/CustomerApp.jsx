...
function ProfileTab({ profile, bookings, favorites, staff }) {
    ...
    <div className="p-4 space-y-4">
      <div className="glass rounded-2xl p-5 border border-white/10">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-brand-600 to-brand-400 flex items-center justify-center text-white text-xl font-bold">
            {(profile?.name || 'C').charAt(0)}
          </div>
          <div>
            <div className="font-semibold text-white">{profile?.name || 'Customer'}</div>
...
export default function CustomerApp() {
  ...
  const { services, staff, bookings, queue, salon, createBooking, loading, error, mode, needsSalonEntry, enterSalonBySlug } = useCustomerAppData(profile);
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              <>
                ...
                {!bookingOpen && activeTab === 'profile' && <ProfileTab profile={profile} bookings={bookings} favorites={favorites} staff={staff} />}
              </>
            )}
          </div>
